...
class _TacticalWOPageState extends State<TacticalWOPage> {
  ...
  Future<void> _saveToFirebase() async {
    ...
    try {
      ...
      // Save status counts
      await _firestore.collection('status_counts').doc('stat_tactical').set({
  ...
  Future<void> _saveData() async {
    try {
      ...
      await prefs.setString('tactical_status_count', json.encode(statusCount));
  ...
  Widget _buildPieChart() {
    ...
    return ModernPieChart(data: statusCount, colors: colors);
  }
    ...
    try {
      ...
      for (int i = 0; i < sheet.maxRows; i++) {
          ...
          for (int j = 0; j < row.length; j++) {
            ...
            if (cellValue.toUpperCase().contains('PM') ||
              ...
              if (cellValue.toUpperCase().startsWith('PM')) {
                typeWO = 'PM';
              } else if (cellValue.toUpperCase().startsWith('CM'))
                typeWO = 'CM';
              break;
            }
          }
        ...
        if (wo.isNotEmpty && (typeWO == 'PM' || typeWO == 'CM')) {
          ...
          print(
            'Added WO: $wo to category: $currentCategory with status: $status, PIC: $pic, Type: $typeWO',
          );
        }
      }
                    ...
                    DataCell(
                      SizedBox(
                        ...
                        child: TextFormField(
                          ...
                          onChanged:
                              (val) =>
                                  _updateRowData(kategori, index, 'wo', val),
                        ),
                      ),
                    ),
                    DataCell(
                        ...
                        child: DropdownButton<String?>(
                          ...
                          onChanged:
                              isEmptyRow
                                  ? null
                                  : (val) =>
                                      _updateStatus(kategori, index, val),
                        ),
                      ),
                    ),
                    DataCell(
                          ...
                          children: [
                            ...
                            IconButton(
                              icon: Icon(
                                ...
                                size: 20,
                                color:
                                    row['photo'] ? Colors.green : Colors.grey,
                              ),
                              onPressed:
                                  isEmptyRow
                                      ? null
