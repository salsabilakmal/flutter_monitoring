...
class _PermintaanPageState extends State<PermintaanPage> {
  ...
  Future<void> _downloadExcel() async {
    try {
      ...
      final List<int>? rawBytes = excelFile.encode();
      if (rawBytes == null) {
        throw Exception('Excel file encoding failed');
      }

      final Uint8List fileBytes = Uint8List.fromList(rawBytes);
      final fileName =
          'Pengambilan_Barang_${DateTime.now().millisecondsSinceEpoch}.xlsx';

      if (kIsWeb) {
        saveExcelFileWeb(fileBytes, fileName);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Export hanya tersedia di Web'),
              ...
              children: [
                ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor:
                        isEditing ? Colors.green.shade700 : Colors.grey,
